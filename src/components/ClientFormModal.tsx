      ...
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-modal overflow-hidden flex flex-col max-h-[90vh] my-auto animate-in fade-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmitForm} className="p-5 overflow-y-auto space-y-4 flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Folio (Opcional)</label>
              <input
                type="text"
                value={folio}
            ...
            <div className="sm:col-span-2 border-t border-slate-100 my-1 pt-3">
              <h3 className="text-xs font-bold text-slate-500 uppercase mb-3">Información Técnica</h3>
            </div>

            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Dirección IP</label>
              <input
                type="text"
                value={ip}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Router / Equipo</label>
              <input
                type="text"
                value={router}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Plan de Internet</label>
              <input
                type="text"
                value={planInternet}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">Tecnología</label>
              <select
                value={esAntena ? 'antena' : 'fibra'}
                onChange={(e) => setEsAntena(e.target.value === 'antena')}
                className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs sm:text-sm font-semibold text-slate-800 focus:ring-2 focus:ring-brand-500"
              >
            ...
            <div className="sm:col-span-2 mt-2">
              <label className="flex items-center gap-2 cursor-pointer p-3 bg-slate-50 border border-slate-200 rounded-xl">
                ...
                />
                <div>
                  <span className="text-sm font-bold text-slate-800 block leading-none">Cliente Activo</span>
                  <span className="text-xs text-slate-500">Si se desactiva, no aparecerá en el autocompletado de reportes.</span>
                </div>
              </label>
            </div>
          </div>
          ...
          {/* Botones de acción */}
          <div className="pt-4 mt-2 border-t border-slate-200 flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
