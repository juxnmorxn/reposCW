...
export const ClientBulkModal: React.FC<ClientBulkModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white w-full max-w-md rounded-3xl shadow-modal overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-bold text-slate-900 text-base sm:text-lg">Acciones Masivas</h2>
              <p className="text-xs text-slate-500">Afecta a múltiples clientes a la vez.</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-5">
          ...
          {/* Acción */}
          <div>
            <label className="block text-sm font-bold text-slate-800 mb-2">¿Qué deseas hacer?</label>
          ...
          {/* Filtro */}
          <div>
            <label className="block text-sm font-bold text-slate-800 mb-2">¿A quiénes aplicar?</label>
            <select
              value={filterType}
              onChange={(e) => setFilterType(e.target.value as any)}
              className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 focus:ring-2 focus:ring-brand-500 mb-3"
            >
            ...
            {filterType === 'router' && (
              <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                <label className="block text-xs font-semibold text-slate-700 mb-1">Selecciona el Equipo (RB)</label>
                <div className="relative">
                  <select
                    value={filterValue}
                    onChange={(e) => setFilterValue(e.target.value)}
                    disabled={isLoadingRouters || routers.length === 0}
                    className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 focus:ring-2 focus:ring-brand-500 disabled:opacity-60"
                  >
                    {routers.length === 0 ? (
